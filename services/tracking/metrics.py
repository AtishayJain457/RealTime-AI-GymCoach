...
def sync_metrics_update(context):
    ...
    event_triggered = False  # Track priority to prevent concurrent API flooding

    # 1. Milestone Event: Set Completed
    if target_sets > 0 and reps_per_set > 0 and sets_completed > last_saved_sets:
        ...
        if st.session_state.get("voice_pipeline"):
            ...
            if result:
                ...
                event_triggered = True  # Block subsequent API hits this frame

        st.session_state.set_cycle_started_at = now_ts
        st.session_state.last_saved_sets_completed = sets_completed

    # 2. Milestone Event: Workout Completed
    if workout_completed and not st.session_state.get("last_notified_workout_complete", False):
        ...
        if st.session_state.get("voice_pipeline") and not event_triggered:
            result = st.session_state.voice_pipeline.process_event(
    ...
    # 3. Boundary Event: No Pose Detected
    pose_detected = latest_metrics.get("pose_detected", True)
    
    if not pose_detected and st.session_state.get("voice_pipeline") and not event_triggered:
        result = st.session_state.voice_pipeline.process_event(
    ...
    # 4. Standard Evaluation Loop: Form Check (Runs only if no major event triggered)
    if st.session_state.get("voice_pipeline") and not event_triggered:
        result = st.session_state.voice_pipeline.process_event(
